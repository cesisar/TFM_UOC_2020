...
def run(argv):
    ...
    get_meteo_variables_cat(oFolder,oFile)
    exit(0)
    
def get_meteo_variables_cat(oFolder,oFile=None): 
    global logger

    # Metadatos variables meteorológicas
    # https://analisi.transparenciacatalunya.cat/Medi-Ambient/Metadades-variables-meteorol-giques/4fb2-n3yi
    dataset_identifier = "4fb2-n3yi"
    with Socrata("analisi.transparenciacatalunya.cat", None) as client:
        filename = oFile if oFile else "meteo_variables.json"
